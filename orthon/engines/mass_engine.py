...
def classify_mass_state(
    mass: MassResult,
    baseline_mass: Optional[float] = None,
) -> Dict:
    """
    Classify current mass state relative to baseline.

    Args:
        mass: Current MassResult
        baseline_mass: Optional baseline for comparison

    Returns:
        Dict with mass state classification
    """
    if baseline_mass is not None and baseline_mass > 1e-10:
        mass_ratio = mass.total_variance / baseline_mass

        if mass_ratio > 2.0:
            state = 'high'
            description = 'Mass significantly elevated (>2x baseline)'
        elif mass_ratio > 1.2:
            state = 'elevated'
            description = 'Mass moderately elevated (1.2-2x baseline)'
        elif mass_ratio < 0.5:
            state = 'depleted'
            description = 'Mass significantly depleted (<0.5x baseline)'
        elif mass_ratio < 0.8:
            state = 'reduced'
            description = 'Mass moderately reduced (0.5-0.8x baseline)'
        else:
            state = 'normal'
            description = 'Mass within normal range'
    else:
        state = 'unknown'
        description = 'No baseline for comparison'
        mass_ratio = 1.0

    return {
        'state': state,
        'description': description,
        'mass_ratio': float(mass_ratio) if baseline_mass else None,
        'total_variance': mass.total_variance,
        'energy_eff_dim': mass.energy_eff_dim,
    }
