...
Usage:
    from orthon.ingest.data_confirmation import confirm_data
...
def confirm_data(
    ...
    if verbose:
        print(f"\n{'='*60}")
        print(f"ORTHON DATA CONFIRMATION")
        print(f"{'='*60}")
        print(f"File: {path}")
        print()
    ...
    if not path.exists():
        result.error("FILE_EXISTS", f"File not found: {path}",
                     "Run ORTHON transform to create observations.parquet")
        if verbose:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="ORTHON Data Confirmation")
