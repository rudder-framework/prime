...
from orthon.shared.physics_constants import (
    PhysicsConstants,
    ENERGY_FORMULAS,
    UNIT_TO_CATEGORY,
...
class SystemDiagnosis:
    ...
    orthon_signal: bool  # The key degradation indicator
    issues: List[str]
    summary: str
...
class PhysicsInterpreter:
    ...
    def analyze_system(self, entity_id: str) -> Dict[str, Any]:
        ...
        The Ørthon signal is detected when all three symptoms appear:
        - Energy dissipating
        - Signals decoupling
        - State diverging
    ...
    def _diagnose(
        ...
        # The Ørthon Signal: all three symptoms together
        ...
        if orthon_signal:
            severity = 'critical'
            issues.insert(0, 'ØRTHON SIGNAL: Symplectic structure loss detected')
    ...
    def analyze_fleet(self) -> Dict[str, Any]:
        ...
        Identifies which entities have the Ørthon signal.
        """
        ...
        for entity in entities:
            try:
                ...
                has_signal = analysis['diagnosis']['orthon_signal']

                results.append({
                    'entity_id': entity,
                    'severity': severity,
                    'orthon_signal': has_signal,
                    'state_distance': analysis['L1_state'].get('current_distance') if analysis.get('L1_state') else None,
                })

                severities.append(severity)
                if has_signal:
                    orthon_signals.append(entity)
    ...
    def get_ml_features(self, entity_id: str, window_size: int = 20) -> Optional[Dict[str, Any]]:
        ...
        current_orthon_signal = 1 if (state_velocity[-1] > 0.1 and coherence[-1] < 0.5) else 0

        risk_score = (
            (current_high_velocity * 30) +
            (current_orthon_signal * 20) +
            (min(accumulators['pct_time_orthon_signal'], 50) * 0.5) +
        ...
        return {
            ...
            **triggers,
            ...
            'current_orthon_signal': current_orthon_signal,
            'orthon_risk_score': round(risk_score, 1),
    ...
    def get_all_ml_features(self, window_size: int = 20) -> List[Dict[str, Any]]:
        ...
        return sorted(features, key=lambda x: x['orthon_risk_score'], reverse=True)
