#!/usr/bin/env python3
"""
Process all domains: Compute typology + Generate manifest v2.4
...
def generate_manifest(typology_path: Path, output_dir: Path, domain_name: str, verbose: bool = True) -> bool:
    ...
    try:
        if verbose:
            print("  [3/3] Generating manifest v2.4...")
...
def process_domain(domain_path: Path, verbose: bool = True) -> bool:
    """
    Process a single domain: typology + manifest.
...
def main():
    parser = argparse.ArgumentParser(
        description='Process all domains: compute typology + generate manifest v2.4'
    )
    parser.add_argument(
        '--domains-dir', '-d',
        default='/Users/jasonrudder/domains',
