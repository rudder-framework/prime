...
SELECT
    ...
    window_idx,
    stability_score,
    ROW_NUMBER() OVER (PARTITION BY entity_id ORDER BY stability_score DESC) AS stability_rank
...
INNER JOIN stable_windows sw
    ON g.entity_id = sw.entity_id
    AND g.I = sw.window_idx
INNER JOIN stable_baseline sb
    ON g.entity_id = sb.entity_id
...
WITH current_state AS (
    ...
    SELECT
        ...
        mutual_info_mean
    FROM geometry
    WHERE (entity_id, I) IN (
        SELECT entity_id, MAX(I) FROM geometry GROUP BY entity_id
    )
),
current_physics AS (
    SELECT
    ...
    FROM physics
    WHERE (entity_id, I) IN (
        SELECT entity_id, MAX(I) FROM physics GROUP BY entity_id
    )
)
SELECT
    c.entity_id,
...
LEFT JOIN v_baseline_geometry b ON c.entity_id = b.entity_id
LEFT JOIN current_physics cp ON c.entity_id = cp.entity_id
LEFT JOIN v_baseline_physics bp ON c.entity_id = bp.entity_id
