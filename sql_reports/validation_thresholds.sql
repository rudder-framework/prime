...
baseline_stats AS (
    SELECT
        ...
        MIN(o.y) AS baseline_min,
        MAX(o.y) AS baseline_max,
        PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY o.y) AS p25,
        PERCENTILE_CONT(0.50) WITHIN GROUP (ORDER BY o.y) AS p50,
        PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY o.y) AS p75
    FROM observations o
...
SELECT
    ...
    -- Clustering validity
    CASE
        WHEN COUNT(DISTINCT entity_id) >= 50 THEN 'Full clustering recommended'
        WHEN COUNT(DISTINCT entity_id) >= 20 THEN 'Basic clustering OK'
        WHEN COUNT(DISTINCT entity_id) >= 10 THEN 'Limited clustering'
        WHEN COUNT(DISTINCT entity_id) >= 5 THEN 'Simple grouping only'
...
WITH entity_stats AS (
    ...
    FROM (
        SELECT entity_id, COUNT(*) AS total_obs_per_entity
        FROM observations
        GROUP BY entity_id
    ) sub
),
